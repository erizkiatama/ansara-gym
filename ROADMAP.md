...
- **Stack**: `chi` (routing/middleware) + `sqlx` over the `pgx/v5/stdlib` driver (Postgres-native performance with familiar `database/sql`/`sqlx` ergonomics) + `slog` (structured logging).
...
- **Topic**: Go project layout (`/cmd`, `/internal`), config via env vars, `chi` router skeleton, `slog` setup, DB connectivity via `sqlx` + `pgx/v5/stdlib`.
...
- **Topic**: Multi-stage `Dockerfile`, `docker-compose` for local dev (app + Postgres + Redis), CI (GitHub Actions: build/test/lint), graceful shutdown on `SIGTERM`, deployment to Fly.io/Render.
- **Feature**: Deployed MVP, reachable over the internet, with CI running on every push.
- **Verify**: `docker build` succeeds and produces a minimal image; local `docker-compose up` runs the full stack; deployed health check responds `200`; CI pipeline green.
